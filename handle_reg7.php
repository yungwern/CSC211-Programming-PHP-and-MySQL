...
// Validate month:
if (empty($_POST['month'] == 'Month')) {
	print'<p class="error">Please enter a valid month.</p>';
	$okay = false;
}
	
// Validate day:
if (empty($_POST['day'] == 'Day')) {
...
if (empty($_POST['year']) || $_POST['year'] == 'YYYY') {
	...
	$okay = false;
// Added check to ensure that string length of numeric value is equal to 4.
} elseif (!is_numeric($_POST['year']) || strlen($_POST['year']) != 4) {
	print '<p class="error">Please enter a valid numeric year and ensure that its 4 digits.</p>';
	$okay = false;	
// I already had this part of the assignment included from previous versions. I did it a bit differently than the book
// but we come to the same conclusion.
} elseif ($_POST['year'] >= 2026) {
	print '<p class="error">Either you entered your birth year wrong or you come from the future!</p>';
	$okay = false;
} else {
	$age = 2026 - $_POST['year']; // Calculate age this year.
}
...
if ($okay) {
	...
	print "<p class='success'>You will turn $age this year.</p>";
}
?>
</body>
</html>